...
async def set_region_task(callback_query: types.CallbackQuery, state: FSMContext=None):
    ...
    callbac_data =  callback_query.data.split(":")
    print(callbac_data)
    state_data = await state.get_data()
    state_data['region'] = callbac_data[0]

    await state.set_data(state_data)

    await callback_query.message.edit_text(
        text=texts.organization,
        reply_markup=buttons.organizators(callbac_data[1])
    )
    await UserSatate.organization.set()
    
@dp.callback_query_handler(state=UserSatate.region)
